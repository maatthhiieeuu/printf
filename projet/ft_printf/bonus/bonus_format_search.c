...
void    traiter_option_numerique(const char *format, int **i, t_format *option) // nom en anglais : process_numeric_option
{
    ...
    int     index;
    char    *point_ptr;
    int number_before_specifier;

    index = **i;
    point_ptr = NULL;
    number_before_specifier = 0;

    if (option->precision == false)
        convertir_espace_chaine_en_entier(format + index, option);
    else if (option->precision == true)
    {
        number_before_specifier = compter_jusquau_specifieur(format + index, option);
        point_ptr = ft_strnchr(format,'.', number_before_specifier);
